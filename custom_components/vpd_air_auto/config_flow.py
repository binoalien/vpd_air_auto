...
class VpdAirAutoOptionsFlow(OptionsFlowWithReload):
    ...
    async def _async_step_select_scope(
        ...
        if not scope_map:
            return self.async_abort(
                reason=self._empty_scope_abort_reason(scope_level)
            )
        if user_input is not None:
    ...
    def _empty_scope_abort_reason(self, scope_level: str) -> str:
        """Return abort reason for an empty area/device policy map."""
        if scope_level == "area":
    ...
    def _scope_id_selector(self, scope_level: str) -> AreaSelector | DeviceSelector:
        """Build scope id selector for area/device policy add forms."""
        if scope_level == "area":
