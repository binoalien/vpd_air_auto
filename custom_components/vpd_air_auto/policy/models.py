"""Dataclass models for V2 policy configuration and resolution."""
...
class DisplayPolicy:
    """Global presentation settings for all derived entities."""
...
@dataclass(frozen=True, slots=True)
class GlobalPolicy:
    """Global behavior defaults plus global display policy."""
...
class ScopedPolicyOverride:
    """Optional behavior overrides for area or device scope."""
...
class SourceOverride:
    """Optional manual source override for one device."""
...
class EffectiveDevicePolicy:
    """Effective resolved policy for one concrete device."""
