"""Declarative sensor definitions for derived helper sensors."""
...
class SensorDefinition:
    """Declarative metadata and value mapping for one sensor kind."""
    ...
    snapshot_value_getter: Callable[[DeviceSnapshot], float | None]
    include_leaf_offset_attribute: bool = False
...
SENSOR_DEFINITIONS: dict[SensorKind, SensorDefinition] = {
    SensorKind.AIR: SensorDefinition(
        ...
        device_class=None,
        snapshot_value_getter=lambda snapshot: snapshot.vpd_air_kpa,
    ),
    SensorKind.LEAF: SensorDefinition(
        ...
        native_unit_of_measurement=UNIT_KPA,
        device_class=None,
        snapshot_value_getter=lambda snapshot: snapshot.vpd_leaf_kpa,
    ...
    SensorKind.ABSOLUTE_HUMIDITY: SensorDefinition(
        ...
        snapshot_value_getter=lambda snapshot: snapshot.absolute_humidity_gm3,
    ),
    SensorKind.DEW_POINT: SensorDefinition(
        ...
        snapshot_value_getter=lambda snapshot: snapshot.dew_point_c,
    ),
}


def get_sensor_definition(kind: SensorKind) -> SensorDefinition:
    """Return the declarative definition for the given sensor kind."""
