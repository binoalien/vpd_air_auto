...
def build_scoped_policy_schema(
    *,
    ...
    schema: dict[Any, Any] = {}
    if include_scope_id:
        selector = AreaSelector(AreaSelectorConfig()) if scope_level == "area" else DeviceSelector(DeviceSelectorConfig())
        schema[vol.Required("scope_id", default=scope_id)] = selector
...
def build_source_override_schema(
    *,
    ...
    schema: dict[Any, Any] = {}
    if include_scope_id:
        schema[vol.Required("scope_id", default=scope_id)] = DeviceSelector(
            DeviceSelectorConfig()
        )
    schema.update(
        {
