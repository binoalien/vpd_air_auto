...
class VpdAirCoordinator(DataUpdateCoordinator[dict[str, DeviceSnapshot]]):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        self._duplicate_detection = DuplicateDetectionService(options)
    ...
    def _discover_topology(self) -> dict[str, DeviceTopology]:
        ...
        for device in device_registry.devices.values():
            ...
            topology[device.id] = DeviceTopology(
                ...
                blocked_sensor_kinds=self._duplicate_detection.detect_existing_derived_sensor_kinds(
                    candidates,
                    self._state_friendly_name,
                    self._entry_device_class,
                ),
            )
