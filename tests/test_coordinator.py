...
async def test_async_update_data_passes_source_overrides_to_discovery(
    ...
    """Coordinator forwards repository source overrides into discovery."""
    coordinator = _build_coordinator(
        hass,
        entry_options={
            "source_overrides": {
    ...
    with (
        patch.object(coordinator._topology_discovery_service, "discover", return_value={}) as mock_discover,
        patch.object(coordinator, "_refresh_state_listener"),
    ):
        await coordinator._async_update_data()

    mock_discover.assert_called_once_with(
        coordinator._policy_repository.source_overrides
    )
