...
def _service(hass: HomeAssistant) -> TopologyDiscoveryService:
    return TopologyDiscoveryService(hass, DuplicateDetectionService(hass, _options()))
...
def test_discover_selects_best_sources_and_blocks_duplicate_sensor_kinds(
    ...
    entries = [
        _entry("sensor.grow_tent_temp_aux", original_name="Aux Temp", original_device_class="temperature", original_unit_of_measurement="°C"),
        _entry("sensor.grow_tent_temperature", original_name="Grow Tent Temperature", original_device_class="temperature", original_unit_of_measurement="°C"),
        _entry("sensor.grow_tent_humidity_generic", original_name="Humidity Generic", original_device_class="humidity", original_unit_of_measurement="%"),
        _entry("sensor.grow_tent_humidity", original_name="Grow Tent Humidity", original_device_class="humidity", original_unit_of_measurement="%"),
        _entry("sensor.grow_tent_vpdair_foreign", original_name="VPDair", original_device_class=None),
        _entry("sensor.grow_tent_leaf_vpd_foreign", original_name="Leaf VPD", original_device_class=None),
        _entry("sensor.grow_tent_absolute_humidity_foreign", original_name="Absolute Humidity", original_device_class="absolute_humidity"),
        _entry("sensor.grow_tent_dew_point_foreign", original_name="Dew Point", original_device_class=None),
        _entry("sensor.own_helper_should_be_ignored", platform=DOMAIN, original_name="VPDair", original_device_class="humidity"),
    ]

    hass.states.async_set("sensor.grow_tent_temperature", "25.0", {"device_class": "temperature", "unit_of_measurement": "°C", "friendly_name": "Grow Tent Temperature"})
    ...
    assert topology["device-1"].blocked_sensor_kinds == frozenset({SENSOR_KIND_AIR, SENSOR_KIND_LEAF, SENSOR_KIND_ABSOLUTE_HUMIDITY, SENSOR_KIND_DEW_POINT})
...
def test_discover_skips_devices_without_complete_source_pair(hass: HomeAssistant) -> None:
    ...
    device_registry = SimpleNamespace(devices={device.id: device})
    entity_registry = SimpleNamespace()
    entries = [_entry("sensor.grow_tent_temperature", original_name="Grow Tent Temperature", original_device_class="temperature", original_unit_of_measurement="°C")]
    hass.states.async_set("sensor.grow_tent_temperature", "25.0", {"device_class": "temperature", "unit_of_measurement": "°C"})
