...
def test_resolver_device_area_global_priority_for_behavior_and_leaf_offset() -> None:
    """Priority must resolve Device > Area > Global for booleans and leaf offset."""
    repository = PolicyRepository(
        {
            "global_policy": {"enable_air": False, "enable_leaf": True, "leaf_offset": -2.0},
            "area_policies": {"grow": {"enable_air": True, "enable_leaf": False, "leaf_offset": -0.5}},
            "device_policies": {"device-1": {"enable_air": False, "leaf_offset": -1.2}},
        }
    )
    effective = PolicyResolver(repository).resolve_for_device(
        device_id="device-1",
