...
async def _start_options_substep(
    hass: HomeAssistant,
    entry: MockConfigEntry,
    menu_step: str,
    action_step: str,
    ...
    flow = await hass.config_entries.options.async_init(entry.entry_id)
    await hass.config_entries.options.async_configure(
        flow["flow_id"], user_input={"next_step_id": menu_step}
    )
    return await hass.config_entries.options.async_configure(
        flow["flow_id"], user_input={"next_step_id": action_step}
    )
...
async def test_area_policy_edit_aborts_when_no_area_policies_configured(
    ...
    result = await _start_options_substep(
