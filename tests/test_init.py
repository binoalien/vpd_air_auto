...
async def test_async_setup_entry_creates_coordinator_and_forwards_platforms(hass: HomeAssistant) -> None:
    ...
    with (
        patch("custom_components.vpd_air_auto.resolve_options", return_value="resolved-options") as mock_resolve,
        patch("custom_components.vpd_air_auto.VpdAirCoordinator", return_value=coordinator) as mock_coordinator,
...
async def test_async_unload_entry_shuts_down_runtime_data_when_platforms_unload(hass: HomeAssistant) -> None:
    ...
    with patch.object(hass.config_entries, "async_unload_platforms", AsyncMock(return_value=True)) as mock_unload:
...
async def test_async_unload_entry_does_not_shutdown_when_platform_unload_fails(hass: HomeAssistant) -> None:
    ...
    with patch.object(hass.config_entries, "async_unload_platforms", AsyncMock(return_value=False)) as mock_unload:
