...
async def test_async_migrate_entry_delegates_to_migration_module(
    hass: HomeAssistant,
) -> None:
    """Test entrypoint migration delegates to migration module."""
    entry = MockConfigEntry(domain=DOMAIN, data={}, version=1)
    entry.add_to_hass(hass)

    with patch(
        "custom_components.vpd_air_auto._async_migrate_entry",
