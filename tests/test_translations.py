...
def test_release_required_translation_error_keys_exist() -> None:
    ...
    required_options_abort_keys = {
        "no_area_policies_configured",
        "no_device_policies_configured",
        "no_source_overrides_configured",
    }

    assert required_options_abort_keys.issubset(set(en["options"]["abort"].keys()))
...
def test_release_gate_workflow_configuration_is_present() -> None:
    """Release-critical CI and Hassfest workflows should keep required gates."""
    ci_text = CI_WORKFLOW_PATH.read_text(encoding="utf-8")
    hassfest_text = HASSFEST_WORKFLOW_PATH.read_text(encoding="utf-8")
